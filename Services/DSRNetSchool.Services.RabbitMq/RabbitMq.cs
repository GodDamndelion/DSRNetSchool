...
public class RabbitMq : IRabbitMq, IDisposable
{
    ...
    private async Task RegisterListener(string queueName, EventHandler<BasicDeliverEventArgs> onReceive, int? messageLifetime = null)
    {
        ...
        AddQueue(queueName, messageLifetime); //Если есть жёстко указанные очереди, то можем подключаться к ним, но перед нами такой задачи не стоит, поэтому можем создавать и т.п.
    ...
    private async Task Publish<T>(string queueName, T data) //Положить данные
    {
        Connect();
    ...
    private void Connect()
    {
        lock (connectionLock)
        {
            ...
            var retriesCount = 0;
            while (retriesCount < 15) //Попробует подключиться 15 раз
                try
                {
                    if (connection == null)
                    {
    ...
    public async Task Subscribe<T>(string queueName, OnDataReceiveEvent<T> onReceive)
    {
        ...
        await RegisterListener(queueName, async (_, eventArgs) =>
        {
            ...
            try
            {
                ...
                await onReceive(obj);
                channel.BasicAck(eventArgs.DeliveryTag, false); //Письмо забрали
            }
            catch (Exception e)
            {
                channel.BasicNack(eventArgs.DeliveryTag, false, false); //Не смогли забрать. RabbitMq либо попытается отдать позже, либо отдать другому
            }
        });
    }
