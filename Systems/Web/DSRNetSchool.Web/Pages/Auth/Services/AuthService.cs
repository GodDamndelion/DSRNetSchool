...
public class AuthService : IAuthService
{
    ...
    public async Task<LoginResult> Login(LoginModel loginModel)
    {
        ...
        var request_body = new[] 
        {
            ...
            new KeyValuePair<string, string>("username", loginModel.Email!),   //Оператор ! - это оператор, допускающий значение null.
            new KeyValuePair<string, string>("password", loginModel.Password!) //Используется, когда нужно указать компилятору, что null либо невозможен, либо ожидается (Чтобы не было предупреждений)
        };
